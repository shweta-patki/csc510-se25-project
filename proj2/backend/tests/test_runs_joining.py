...
def create_run(client, token, restaurant="Port City Java EBII", drop="Wolf Ridge", capacity=2, eta="10:30"):
    payload = {
...
def test_capacity_prevents_join_when_full(app_client):
    ...
    run = create_run(app_client, runner_token, capacity=1)
    run_id = run["id"]
    app_client.post(f"/runs/{run_id}/orders", headers={"Authorization": f"Bearer {u1_token}"}, json={"items": "x", "amount": 1.0})
...
def test_unjoin_allows_another_join(app_client):
    ...
    run_id = run["id"]
    app_client.post(f"/runs/{run_id}/orders", headers={"Authorization": f"Bearer {u1_token}"}, json={"items": "x", "amount": 1.0})
    uj = app_client.delete(f"/runs/{run_id}/orders/me", headers={"Authorization": f"Bearer {u1_token}"})
...
def test_runner_orders_do_not_include_pins(app_client):
    ...
    rm = app_client.get("/runs/mine", headers={"Authorization": f"Bearer {runner_token}"})
...
def test_join_response_includes_4digit_pin(app_client):
    ...
    body = rj.json()
    assert "pin" in body and isinstance(body["pin"], str) and len(body["pin"]) == 4 and body["pin"].isdigit()
...
def test_unjoin_without_existing_order_returns_404(app_client):
    ...
    run = create_run(app_client, runner_token)
    ru = app_client.delete(f"/runs/{run['id']}/orders/me", headers={"Authorization": f"Bearer {user_token}"})
...
def test_available_excludes_runner_and_full_runs(app_client):
    ...
    r_av_runner = app_client.get("/runs/available", headers={"Authorization": f"Bearer {runner_token}"})
    ...
    assert rj.status_code in (200, 201)
    r_av_other = app_client.get("/runs/available", headers={"Authorization": f"Bearer {other_token}"})

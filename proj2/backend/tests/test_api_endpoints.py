...
def test_09_create_run_success(client):
    ...
    payload = {"restaurant": "Taco Place", "drop_point": "Bldg A", "eta": "12:00", "capacity": 4}
...
def test_12_list_runs(client):
    ...
    client.post("/runs", json={"restaurant": "A", "drop_point": "B", "eta": "1pm", "capacity": 3}, headers=auth_headers(token))
...
def test_13_get_run_details_authorized(client):
    ...
    create = client.post("/runs", json={"restaurant": "X", "drop_point": "Y", "eta": "t", "capacity": 2}, headers=auth_headers(token))
...
def test_14_get_run_details_unauthorized(client):
    ...
    run = client.post("/runs", json={"restaurant": "Z", "drop_point": "D", "eta": "t", "capacity": 2}, headers=auth_headers(owner_token)).json()
    run_id = run["id"]
...
def test_15_join_run_success(client):
    ...
    run = client.post("/runs", json={"restaurant": "J", "drop_point": "K", "eta": "t", "capacity": 3}, headers=auth_headers(owner_token)).json()
    run_id = run["id"]
    register(client, "joiner@ncsu.edu", "pw")
    join_token = login(client, "joiner@ncsu.edu", "pw").json()["token"]
    r = client.post(f"/runs/{run_id}/orders", json={"items": "[]", "amount": 15.5}, headers=auth_headers(join_token))
    assert r.status_code == 200
    body = r.json()
...
def test_16_join_run_full(client):
    ...
    run = client.post("/runs", json={"restaurant": "F", "drop_point": "G", "eta": "t", "capacity": 1}, headers=auth_headers(owner_token)).json()
    run_id = run["id"]
    # one joiner fills capacity
    register(client, "j1@ncsu.edu", "pw")
    t1 = login(client, "j1@ncsu.edu", "pw").json()["token"]
    client.post(f"/runs/{run_id}/orders", json={"items": "[]", "amount": 5.0}, headers=auth_headers(t1))
    # second join attempt
    register(client, "j2@ncsu.edu", "pw")
    t2 = login(client, "j2@ncsu.edu", "pw").json()["token"]
    r = client.post(f"/runs/{run_id}/orders", json={"items": "[]", "amount": 7.0}, headers=auth_headers(t2))
...
def test_17_runner_cannot_join_own_run(client):
    ...
    run = client.post("/runs", json={"restaurant": "S", "drop_point": "P", "eta": "t", "capacity": 2}, headers=auth_headers(token)).json()
    r = client.post(f"/runs/{run['id']}/orders", json={"items": "[]", "amount": 10}, headers=auth_headers(token))
    assert r.status_code == 400
...
def test_18_verify_pin_success(client):
    ...
    run = client.post("/runs", json={"restaurant": "V", "drop_point": "Q", "eta": "t", "capacity": 3}, headers=auth_headers(owner_token)).json()
    run_id = run["id"]
    register(client, "u5@ncsu.edu", "pw")
    t5 = login(client, "u5@ncsu.edu", "pw").json()["token"]
    order = client.post(f"/runs/{run_id}/orders", json={"items": "[]", "amount": 8.0}, headers=auth_headers(t5)).json()
    order_id = order["id"]
    pin = order["pin"]
    r = client.post(f"/runs/{run_id}/orders/{order_id}/verify-pin", json={"pin": pin}, headers=auth_headers(owner_token))
    assert r.status_code == 200
...
def test_19_verify_pin_wrong(client):
    ...
    run = client.post("/runs", json={"restaurant": "W", "drop_point": "R", "eta": "t", "capacity": 3}, headers=auth_headers(owner_token)).json()
    run_id = run["id"]
    register(client, "u6@ncsu.edu", "pw")
    t6 = login(client, "u6@ncsu.edu", "pw").json()["token"]
    order = client.post(f"/runs/{run_id}/orders", json={"items": "[]", "amount": 9.0}, headers=auth_headers(t6)).json()
    order_id = order["id"]
    r = client.post(f"/runs/{run_id}/orders/{order_id}/verify-pin", json={"pin": "0000"}, headers=auth_headers(owner_token))
    assert r.status_code == 400
...
def test_20_cancel_my_order(client):
    ...
    run = client.post("/runs", json={"restaurant": "C", "drop_point": "D", "eta": "t", "capacity": 3}, headers=auth_headers(owner_tok)).json()
    run_id = run["id"]
    register(client, "canceler@ncsu.edu", "pw")
    t = login(client, "canceler@ncsu.edu", "pw").json()["token"]
    client.post(f"/runs/{run_id}/orders", json={"items": "[]", "amount": 6.0}, headers=auth_headers(t))
...
def test_21_runner_remove_order(client):
    ...
    run = client.post("/runs", json={"restaurant": "L", "drop_point": "M", "eta": "t", "capacity": 3}, headers=auth_headers(owner_tok)).json()
    run_id = run["id"]
    register(client, "o9@ncsu.edu", "pw")
    t9 = login(client, "o9@ncsu.edu", "pw").json()["token"]
    order = client.post(f"/runs/{run_id}/orders", json={"items": "[]", "amount": 3.0}, headers=auth_headers(t9)).json()
    r = client.delete(f"/runs/{run_id}/orders/{order['id']}", headers=auth_headers(owner_tok))
    assert r.status_code == 200
...
def test_22_complete_run_awards_integer_points(client):
    ...
    run = client.post("/runs", json={"restaurant": "P", "drop_point": "Z", "eta": "t", "capacity": 5}, headers=auth_headers(owner_tok)).json()
    run_id = run["id"]
    ...
    client.post(f"/runs/{run_id}/orders", json={"items": "[]", "amount": 20.25}, headers=auth_headers(t10))
    register(client, "u11@ncsu.edu", "pw")
    t11 = login(client, "u11@ncsu.edu", "pw").json()["token"]
    client.post(f"/runs/{run_id}/orders", json={"items": "[]", "amount": 22.5}, headers=auth_headers(t11))
...
def test_23_complete_run_unauthorized(client):
    ...
    run = client.post("/runs", json={"restaurant": "Q", "drop_point": "R", "eta": "t", "capacity": 2}, headers=auth_headers(owner_tok)).json()
    run_id = run["id"]
...
def test_24_list_available_runs_filters(client):
    ...
    run = client.post("/runs", json={"restaurant": "Avail", "drop_point": "Here", "eta": "t", "capacity": 1}, headers=auth_headers(owner_tok)).json()
    run_id = run["id"]
    register(client, "j12@ncsu.edu", "pw")
    t = login(client, "j12@ncsu.edu", "pw").json()["token"]
    # join to make it full
    client.post(f"/runs/{run_id}/orders", json={"items": "[]", "amount": 4.0}, headers=auth_headers(t))
...
def test_26_redeem_points_success(client):
    ...
    # create run as user
    run = client.post("/runs", json={"restaurant": "X", "drop_point": "Y", "eta": "t", "capacity": 1}, headers=auth_headers(t)).json()
    run_id = run["id"]
    # join as another user
    register(client, "payer@ncsu.edu", "pw")
    tp = login(client, "payer@ncsu.edu", "pw").json()["token"]
    client.post(f"/runs/{run_id}/orders", json={"items": "[]", "amount": 100.0}, headers=auth_headers(tp))
...
def test_28_joined_runs_pin_exposure(client):
    ...
    run = client.post("/runs", json={"restaurant": "Pin", "drop_point": "PP", "eta": "t", "capacity": 3}, headers=auth_headers(owner_tok)).json()
    run_id = run["id"]
    register(client, "joinx@ncsu.edu", "pw")
    tx = login(client, "joinx@ncsu.edu", "pw").json()["token"]
    order = client.post(f"/runs/{run_id}/orders", json={"items": "[]", "amount": 2.0}, headers=auth_headers(tx)).json()
    # joined runs for joiner should expose my_order.pin
    r = client.get("/runs/joined", headers=auth_headers(tx))
    assert r.status_code == 200
    joined = r.json()
    assert any((entry.get("my_order") and entry["my_order"].get("pin") is not None) for entry in joined)
...
def test_29_runs_history_for_runner(client):
    ...
    run = client.post("/runs", json={"restaurant": "H", "drop_point": "I", "eta": "t", "capacity": 2}, headers=auth_headers(tok)).json()
    run_id = run["id"]
    register(client, "hjoin@ncsu.edu", "pw")
    th = login(client, "hjoin@ncsu.edu", "pw").json()["token"]
    client.post(f"/runs/{run_id}/orders", json={"items": "[]", "amount": 3.0}, headers=auth_headers(th))
...
def test_30_create_order_invalid_amount(client):
    ...
    run = client.post("/runs", json={"restaurant": "Bad", "drop_point": "B", "eta": "t", "capacity": 2}, headers=auth_headers(xo)).json()
    run_id = run["id"]
    # invalid type
    r1 = client.post(f"/runs/{run_id}/orders", json={"items": "[]", "amount": "nope"}, headers=auth_headers(to))
    assert r1.status_code == 422
    # negative amount (should be rejected or handled)
    r2 = client.post(f"/runs/{run_id}/orders", json={"items": "[]", "amount": -5}, headers=auth_headers(to))
